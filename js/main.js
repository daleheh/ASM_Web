...
function detailProduct(index) {
    ...
    productbtn.addEventListener('click', (e) => {
        if (localStorage.getItem('currentuser')) {
            ...
            toast({ title: 'Warning', message: 'Not logged in account !', type: 'warning', duration: 3000 });
        }
...
signupButton.addEventListener('click', () => {
    ...
    if (fullNameUser.length == 0) {
        document.querySelector('.form-message-name').innerHTML = 'Please enter your first and last name';
        document.getElementById('fullname').focus();
    } else if (fullNameUser.length < 3) {
        document.getElementById('fullname').value = '';
        document.querySelector('.form-message-name').innerHTML = 'Please enter your full name greater than 3 characters';
    } else {
        document.querySelector('.form-message-name').innerHTML = '';
    }
    if (phoneUser.length == 0) {
        document.querySelector('.form-message-phone').innerHTML = 'Please enter phone number';
    } else if (phoneUser.length != 10) {
        document.querySelector('.form-message-phone').innerHTML = 'Please enter 10 digit phone number';
        document.getElementById('phone').value = '';
    } else {
        document.querySelector('.form-message-phone').innerHTML = '';
    }
    if (passwordUser.length == 0) {
        document.querySelector('.form-message-password').innerHTML = 'Please enter password';
    } else if (passwordUser.length < 6) {
        document.querySelector('.form-message-password').innerHTML = 'Please enter a password greater than 6 characters';
        document.getElementById('password').value = '';
    } else {
        document.querySelector('.form-message-password').innerHTML = '';
    }
    if (passwordConfirmation.length == 0) {
        document.querySelector('.form-message-password-confi').innerHTML = 'Please re-enter password';
    } else if (passwordConfirmation !== passwordUser) {
        document.querySelector('.form-message-password-confi').innerHTML = 'Passwords do not match';
        document.getElementById('password_confirmation').value = '';
    } else {
        document.querySelector('.form-message-password-confi').innerHTML = '';
    }
    if (checkSignup != true) {
        document.querySelector('.form-message-checkbox').innerHTML = 'Please check registration';
    } else {
    ...
    if (fullNameUser && phoneUser && passwordUser && passwordConfirmation && checkSignup) {
        if (passwordConfirmation == passwordUser) {
            ...
            if (!checkloop) {
                ...
                toast({ title: 'Thành công', message: 'Account created successfully!', type: 'success', duration: 3000 });
                closeModal();
                kiemtradangnhap();
                updateAmount();
            } else {
                toast({ title: 'Thất bại', message: 'Account already exists!', type: 'error', duration: 3000 });
            }
        } else {
            toast({ title: 'Thất bại', message: 'Wrong password!', type: 'error', duration: 3000 });
        }
    }
}
)
...
loginButton.addEventListener('click', () => {
    ...
    if (phonelog.length == 0) {
        document.querySelector('.form-message.phonelog').innerHTML = 'Please enter phone number';
    } else if (phonelog.length != 10) {
        document.querySelector('.form-message.phonelog').innerHTML = 'Please enter 10 digit phone number';
    ...
    if (passlog.length == 0) {
        document.querySelector('.form-message-check-login').innerHTML = 'Please enter password';
    } else if (passlog.length < 6) {
        document.querySelector('.form-message-check-login').innerHTML = 'Please enter a password greater than 6 characters';
    ...
    if (phonelog && passlog) {
        ...
        if (vitri == -1) {
            toast({ title: 'Error', message: 'Your account does not exist', type: 'error', duration: 3000 });
        } else if (accounts[vitri].password == passlog) {
            if(accounts[vitri].status == 0) {
                toast({ title: 'Warning', message: 'Your account has been locked', type: 'warning', duration: 3000 });
            } else {
            ...
            toast({ title: 'Warning', message: 'Wrong password', type: 'warning', duration: 3000 });
        }
    }
})
...
function kiemtradangnhap() {
    ...
    if (currentUser != null) {
        ...
        document.querySelector('.header-middle-right-menu').innerHTML = `<li><a href="javascript:;" onclick="myAccount()"><i class="fa-light fa-circle-user"></i> My account</a></li>
            ...
            <li class="border"><a id="logout" href="javascript:;"><i class="fa-light fa-right-from-bracket"></i> Log out of account</a></li>`
...
function changeInformation() {
    ...
    if (infoemail.value.length > 0) {
        if (!emailIsValid(infoemail.value)) {
            document.querySelector('.inforemail-error').innerHTML = 'Please re-enter email!';
            infoemail.value = '';
        } else {
    ...
    localStorage.setItem('accounts', JSON.stringify(accounts));
    kiemtradangnhap();
    toast({ title: 'Success', message: 'Information updated successfully!', type: 'success', duration: 3000 });
}
...
function changePassword() {
    ...
    if (passwordCur.value.length == 0) {
        document.querySelector('.password-cur-info-error').innerHTML = 'Please enter current password';
        check = false;
    } else {
    ...
    if (passwordAfter.value.length == 0) {
        document.querySelector('.password-after-info-error').innerHTML = 'Please enter new password';
        check = false;
    } else {
    ...
    if (passwordConfirm.value.length == 0) {
        document.querySelector('.password-after-comfirm-error').innerHTML = 'Please enter confirmation password';
        check = false;
    } else {
            ...
            if (passwordCur.value == currentUser.password) {
                ...
                if (passwordAfter.value.length > 0) {
                    if (passwordAfter.value.length < 6) {
                        document.querySelector('.password-after-info-error').innerHTML = 'Please enter a new password with more than 6 characters';
                    } else {
                        document.querySelector('.password-after-info-error').innerHTML = '';
                        if (passwordConfirm.value.length > 0) {
                            if (passwordConfirm.value == passwordAfter.value) {
                                ...
                                document.querySelector('.password-after-comfirm-error').innerHTML = 'The password you entered does not match';
                            }
                        } else {
                            document.querySelector('.password-after-comfirm-error').innerHTML = 'Please confirm password';
                        }
                    }
                } else {
                    document.querySelector('.password-after-info-error').innerHTML = 'Please enter new password';
                ...
                document.querySelector('.password-cur-info-error').innerHTML = 'You have entered an incorrect current password';
            }
        }
    }
}
...
function renderOrderProduct() {
    ...
    if (arrDonHang.length == 0) {
        ...
        arrDonHang.forEach(item => {
            ...
            let textCompl = item.trangthai == 1 ? "Đã xử lý" : "Procceeding";
            ...
            productHtml += `<div class="order-history-control">
                ...
                <div class="order-history-total">
                    <span class="order-history-total-desc">Total price: </span>
...
function detailOrder(id) {
    ...
    let detailOrderHtml = `<ul class="detail-order-group">
        ...
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-light fa-truck"></i> Delivery form</span>
            <span class="detail-order-item-right">${detail.hinhthucgiao}</span>
        </li>
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-light fa-clock"></i> Date of receipt</span>
            <span class="detail-order-item-right">${(detail.thoigiangiao == "" ? "" : (detail.thoigiangiao + " - ")) + formatDate(detail.ngaygiaohang)}</span>
        </li>
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-light fa-location-dot"></i> Pick up location</span>
            <span class="detail-order-item-right">${detail.diachinhan}</span>
        </li>
        <li class="detail-order-item">
            <span class="detail-order-item-left"><i class="fa-thin fa-person"></i> Receiver</span>
...
function renderProducts(showProduct) {
    ...
    if(showProduct.length == 0) {
        ...
        productHtml = `<div class="no-result"><div class="no-result-h">Search returned no results</div><div class="no-result-p">Sorry, we couldn't find any results that match your search.</div><div class="no-result-i"><i class="fa-light fa-face-sad-cry"></i></div></div>`;
    } else {
