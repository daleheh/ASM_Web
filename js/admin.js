...
function deleteProduct(id) {
    ...
    if (confirm("Are you sure you want to delete?") == true) {
        ...
        toast({ title: 'Success', message: 'Product deleted successfully!', type: 'success', duration: 3000 });
    }
...
function changeStatusProduct(id) {
    ...
    if (confirm("Are you sure you want to cancel deletion?") == true) {
        products[index].status = 1;
        toast({ title: 'Success', message: 'Product restore successful!', type: 'success', duration: 3000 });
    }
    localStorage.setItem("products", JSON.stringify(products));
    showProduct();
}
...
btnUpdateProductIn.addEventListener("click", (e) => {
    ...
    if (imgProductCur != imgProduct || titleProductCur != titleProduct || curProductCur != curProduct || descProductCur != descProduct || categoryText != categoryProduct) {
        ...
        localStorage.setItem("products", JSON.stringify(products));
        toast({ title: "Success", message: "Product repair successful!", type: "success", duration: 3000, });
        setDefaultValue();
        document.querySelector(".add-product").classList.remove("open");
        showProduct();
    } else {
        toast({ title: "Warning", message: "Your product remains unchanged!", type: "warning", duration: 3000, });
    }
});
...
btnAddProductIn.addEventListener("click", (e) => {
    ...
    if(tenMon == "" || price == "" || moTa == "") {
        toast({ title: "Attention", message: "Please enter full item information!", type: "warning", duration: 3000, });
    } else {
        if(isNaN(price)) {
            toast({ title: "Attention", message: "Price must be in number!", type: "warning", duration: 3000, });
        } else {
            let products = localStorage.getItem("products") ? JSON.parse(localStorage.getItem("products")) : [];
            let product = {
            ...
            showProduct();
            document.querySelector(".add-product").classList.remove("open");
            toast({ title: "Success", message: "Product added successfully!", type: "success", duration: 3000});
            setDefaultValue();
        }
    }
});
...
function showOrder(arr) {
    let orderHtml = "";
    if(arr.length == 0) {
        orderHtml = `<td colspan="6">No data available</td>`
    } else {
        arr.forEach((item) => {
...
function detailOrder(id) {
    ...
    let textDetailBtn = order.trangthai == 0 ? "Not processed" : "Processed";
    document.querySelector(
        ".modal-detail-bottom"
    ).innerHTML = `<div class="modal-detail-bottom-left">
        <div class="price-total">
            <span class="thanhtien">Total amount</span>
...
function showUserArr(arr) {
    let accountHtml = '';
    if(arr.length == 0) {
        accountHtml = `<td colspan="5">No data</td>`
    } else {
...
updateAccount.addEventListener("click", (e) => {
    ...
    if(fullname == "" || phone == "" || password == "") {
        toast({ title: 'Attention', message: 'Please enter complete information!', type: 'warning', duration: 3000 });
    } else {
        ...
        localStorage.setItem("accounts", JSON.stringify(accounts));
        toast({ title: 'Successfully', message: 'Change information successfully!', type: 'success', duration: 3000 });
...
addAccount.addEventListener("click", (e) => {
    ...
    let passwordUser = document.getElementById('password').value;
        ...
        if (fullNameUser.length == 0) {
            formMessageName.innerHTML = 'Please enter your full name';
            fullNameIP.focus();
        } else if (fullNameUser.length < 3) {
            fullNameIP.value = '';
            formMessageName.innerHTML = 'Please enter a first and last name greater than 3 characters';
        }
        
        if (phoneUser.length == 0) {
            formMessagePhone.innerHTML = 'Please enter phone number';
        } else if (phoneUser.length != 10) {
            formMessagePhone.innerHTML = 'Please enter 10 digit phone number';
            document.getElementById('phone').value = '';
        }
        
        if (passwordUser.length == 0) {
            formMessagePassword.innerHTML = 'Please enter password';
        } else if (passwordUser.length < 6) {
            formMessagePassword.innerHTML = 'Please enter a password greater than 6 characters';
            document.getElementById('password').value = '';
        }
    ...
    if (fullNameUser && phoneUser && passwordUser) {
        ...
        if (!checkloop) {
            accounts.push(user);
            localStorage.setItem('accounts', JSON.stringify(accounts));
            toast({ title: 'Successfully', message: 'Account created successfully!', type: 'success', duration: 3000 });
            document.querySelector(".signup").classList.remove("open");
            showUser();
            signUpFormReset();
        } else {
            toast({ title: 'Warning!', message: 'Account already exists!', type: 'error', duration: 3000 });
        }
    }
})
